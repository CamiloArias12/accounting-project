"""seed colombian municipalities

Loads the 1122 rows of the DANE Divipola into `cities`, attaching each one to
its department through the first two digits of its code.

Source: `DIVIPOLA- Códigos municipios`, dataset gdxc-w37w of datos.gov.co,
downloaded 2026-07-26. The 1122 rows are 1103 municipalities, 18 `áreas no
municipalizadas` and one island; all of them are places a document can be issued
in or a person can live in, so all of them are loaded. The DANE ships the names
in upper case; they are stored title-cased.

Written out in full, for the same reason as the previous revision: a migration
that imports application code stops reproducing what it originally produced.

Revision ID: d204a471ea58
Revises: 7355399db43d
Create Date: 2026-07-26 04:50:55.521349
...
revision: str = "d204a471ea58"
down_revision: str | None = "7355399db43d"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

_COLOMBIA = "CO"
#: How many digits of a municipality code identify its department.
_DEPARTMENT_PREFIX = 2

_MUNICIPALITIES: tuple[tuple[str, str], ...] = (
    ("05001", "Medellín"),
    ("05002", "Abejorral"),
    ("05004", "Abriaquí"),
    ("05021", "Alejandría"),
...
def upgrade() -> None:
    ...
    colombia_id = bind.execute(
        sa.select(_countries.c.id).where(_countries.c.iso_code == _COLOMBIA)
    ).scalar_one()

    department_ids = {
        dane_code: department_id
        for department_id, dane_code in bind.execute(
            sa.select(_departments.c.id, _departments.c.dane_code).where(
                _departments.c.country_id == colombia_id
            )
        )
    }
    ...
    op.bulk_insert(
        ...
        [
            {
                # Raises KeyError on an unknown prefix rather than inserting a
                # municipality under the wrong department.
                "department_id": department_ids[dane_code[:_DEPARTMENT_PREFIX]],
                "dane_code": dane_code,
                "name": name,
            }
            for dane_code, name in _MUNICIPALITIES
        ],
    )
